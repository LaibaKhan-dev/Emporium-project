...
function ProductModal({reCallToData}) {
    ...
    const MultipleImageUpload = () => images?.map((val) => {
        const MultipleImageRef = ref(storage, `/images/products/${productName}/${val.productName}`);
    ...
    const AddProduct = (e) => {
        ...
        Promise.all(uploadImages)
            .then(() => {
                const storageRef = ref(storage, `/images/products/${productName}/${thumbnail.productName}`);
                uploadBytes(storageRef, thumbnail).then((snapshot) => {
                    getDownloadURL(snapshot.ref)
                        .then((url) => {
                            ...
                            axios.post(`${AppRoute}api/products`, payload).then((json) => {
            ...
            <Modal show={show} onHide={handleClose} centered backdrop="static" size="lg">
                ...
                <Modal.Body>
                    <form onSubmit={AddProduct}>
                        <div className="row">
                            <div className="col">
                                <FloatingLabel controlId="name" label="Product Name" className="mb-3 text-secondary"                                >
